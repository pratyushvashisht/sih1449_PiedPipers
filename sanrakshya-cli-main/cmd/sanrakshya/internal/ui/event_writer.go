...
import (
	...
	"github.com/anubhav06/sanrakshya-cli/sanrakshya/event/parsers"
)
...
func writeAppUpdate(writer io.Writer, events ...partybus.Event) error {
	...
	for _, e := range events {
		...
		notice := fmt.Sprintf("A newer version of sanrakshya is available for download: %s (installed version is %s)", updateCheck.New, updateCheck.Current)
