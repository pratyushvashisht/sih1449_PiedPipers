...
import (
	...
	"github.com/anubhav06/sanrakshya-cli/sanrakshya/format/common"
...
func encodeComponent(p pkg.Package) cyclonedx.Component {
	props := encodeProperties(p, "sanrakshya:package")
	...
	if len(locations) > 0 {
		props = append(props, encodeProperties(locations, "sanrakshya:location")...)
	}
	if hasMetadata(p) {
		props = append(props, encodeProperties(p.Metadata, "sanrakshya:metadata")...)
	}
...
func deriveBomRef(p pkg.Package) string {
	// try and parse the PURL if possible and append sanrakshya id to it, to make
...
func decodeComponent(c *cyclonedx.Component) *pkg.Package {
	...
	common.DecodeInto(p, values, "sanrakshya:package", CycloneDXFields)

	metadataType := values["sanrakshya:package:metadataType"]
...
func decodeLocations(vals map[string]string) file.LocationSet {
	v := common.Decode(reflect.TypeOf([]file.Location{}), vals, "sanrakshya:location", CycloneDXFields)
...
func decodePackageMetadata(vals map[string]string, c *cyclonedx.Component, typeName string) interface{} {
	if typeName != "" && c.Properties != nil {
		...
		metaPtr := common.Decode(metaPtrTyp, vals, "sanrakshya:metadata", CycloneDXFields)
