...
func toOSComponent(distro *linux.Release) []cyclonedx.Component {
	...
	props := encodeProperties(distro, "sanrakshya:distro")
...
// NewBomDescriptor returns a new BomDescriptor tailored for the current time and "sanrakshya" tool details.
...
// used to indicate that a relationship listed under the sanrakshya artifact package can be represented as a cyclonedx dependency.
...
func toBomProperties(srcMetadata source.Description) *[]cyclonedx.Property {
	...
	if ok {
		props := encodeProperties(metadata.Labels, "sanrakshya:image:labels")
		return &props
	}
	return nil
}
