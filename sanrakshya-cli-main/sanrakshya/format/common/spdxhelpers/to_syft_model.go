...
import (
	...
	"github.com/anubhav06/sanrakshya-cli/sanrakshya/format/common/util"
...
func TosanrakshyaModel(doc *spdx.Document) (*sbom.SBOM, error) {
	if doc == nil {
		return nil, errors.New("cannot convert SPDX document to sanrakshya model because document is nil")
	}
	...
	collectsanrakshyaPackages(s, spdxIDMap, doc.Packages)

	collectsanrakshyaFiles(s, spdxIDMap, doc)

	s.Relationships = tosanrakshyaRelationships(spdxIDMap, doc)
...
func fileSource(p *spdx.Package) source.Description {
	...
	case typeName == prefixDirectory:
		// is a sanrakshya SBOM, explicitly a directory source
		metadata, version = directorySourceMetadata(p)
	case typeName == prefixFile:
		// is a sanrakshya SBOM, explicitly a file source
		metadata, version = fileSourceMetadata(p)
	case isDirectory(p.PackageName):
		// is a non-sanrakshya SBOM, which looks like a directory
		metadata, version = directorySourceMetadata(p)
	default:
		// is a non-sanrakshya SBOM, which is probably a file
		metadata, version = fileSourceMetadata(p)
	}
...
func fileSourceMetadata(p *spdx.Package) (any, string) {
	...
	// if this is a sanrakshya SBOM, we might have output a digest as the version
...
// the scheme. sanrakshya-generated SBOMs have in the namespace
...
func collectsanrakshyaPackages(s *sbom.SBOM, spdxIDMap map[string]any, packages []*spdx.Package) {
	for _, p := range packages {
		sanrakshyaPkg := tosanrakshyaPackage(p)
		spdxIDMap[string(p.PackageSPDXIdentifier)] = sanrakshyaPkg
		s.Artifacts.Packages.Add(sanrakshyaPkg)
	}
}

func collectsanrakshyaFiles(s *sbom.SBOM, spdxIDMap map[string]any, doc *spdx.Document) {
	for _, p := range doc.Packages {
		for _, f := range p.Files {
			l := tosanrakshyaLocation(f)
...
func toFileMetadata(f *spdx.File) (meta file.Metadata) {
	// FIXME sanrakshya is currently lossy due to the SPDX 2.2.1 spec not supporting arbitrary mimetypes
...
func tosanrakshyaRelationships(spdxIDMap map[string]any, doc *spdx.Document) []artifact.Relationship {
...
func tosanrakshyaCoordinates(f *spdx.File) file.Coordinates {
...
func tosanrakshyaLocation(f *spdx.File) file.Location {
	l := file.NewVirtualLocationFromCoordinates(tosanrakshyaCoordinates(f), f.FileName)
	return l
}
...
func tosanrakshyaPackage(p *spdx.Package) pkg.Package {
