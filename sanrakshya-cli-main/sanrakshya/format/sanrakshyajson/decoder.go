...
func (d decoder) Decode(reader io.ReadSeeker) (*sbom.SBOM, sbom.FormatID, string, error) {
	...
	if version == "" || id != ID {
		return nil, "", "", fmt.Errorf("not a sanrakshya-json document")
	}
	var doc model.Document

	if _, err := reader.Seek(0, io.SeekStart); err != nil {
		return nil, "", "", fmt.Errorf("unable to seek to start of sanrakshya JSON SBOM: %+v", err)
	}
	...
	err := dec.Decode(&doc)
	if err != nil {
		return nil, "", "", fmt.Errorf("unable to decode sanrakshya-json document: %w", err)
	}
	...
	return tosanrakshyaModel(doc), ID, doc.Schema.Version, nil
}
...
func (d decoder) Identify(reader io.ReadSeeker) (sbom.FormatID, string) {
	...
	if _, err := reader.Seek(0, io.SeekStart); err != nil {
		log.Debugf("unable to seek to start of sanrakshya JSON SBOM: %+v", err)
		return "", ""
	}
	...
	if !strings.Contains(doc.Schema.URL, "anchore/sanrakshya") {
		// not a sanrakshya-json document
		return "", ""
	}
