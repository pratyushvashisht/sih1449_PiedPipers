...
func tosanrakshyaModel(doc model.Document) *sbom.SBOM {
	idAliases := make(map[string]string)

	catalog := tosanrakshyaCatalog(doc.Artifacts, idAliases)

	fileArtifacts := tosanrakshyaFiles(doc.Files)
	...
	return &sbom.SBOM{
		Artifacts: sbom.Artifacts{
			...
			LinuxDistribution: tosanrakshyaLinuxRelease(doc.Distro),
		},
		Source:        *tosanrakshyaSourceData(doc.Source),
		Descriptor:    tosanrakshyaDescriptor(doc.Descriptor),
		Relationships: warnConversionErrors(tosanrakshyaRelationships(&doc, catalog, doc.ArtifactRelationships, idAliases)),
	}
}
...
func tosanrakshyaFiles(files []model.File) sbom.Artifacts {
	ret := sbom.Artifacts{
	...
	for _, f := range files {
		...
		if f.Metadata != nil {
			...
			ret.FileMetadata[coord] = file.Metadata{
				...
				Type:            tosanrakshyaFileType(f.Metadata.Type),
...
func tosanrakshyaLicenses(m []model.License) (p []pkg.License) {
	for _, l := range m {
...
func tosanrakshyaFileType(ty string) stereoscopeFile.Type {
	switch ty {
	case "SymbolicLink":
...
func tosanrakshyaLinuxRelease(d model.LinuxRelease) *linux.Release {
...
func tosanrakshyaRelationships(doc *model.Document, catalog *pkg.Collection, relationships []model.Relationship, idAliases map[string]string) ([]artifact.Relationship, []error) {
	...
	idMap[doc.Source.ID] = tosanrakshyaSource(doc.Source)
	...
	for _, r := range relationships {
		sanrakshyaRelationship, err := tosanrakshyaRelationship(idMap, r, idAliases)
		if err != nil {
			conversionErrors = append(conversionErrors, err)
		}
		if sanrakshyaRelationship != nil {
			out = append(out, *sanrakshyaRelationship)
		}
	}
...
func tosanrakshyaSource(s model.Source) source.Source {
	description := tosanrakshyaSourceData(s)
	if description == nil {
		return nil
	}
...
func tosanrakshyaRelationship(idMap map[string]interface{}, relationship model.Relationship, idAliases map[string]string) (*artifact.Relationship, error) {
...
func tosanrakshyaDescriptor(d model.Descriptor) sbom.Descriptor {
...
func tosanrakshyaSourceData(s model.Source) *source.Description {
...
func tosanrakshyaCatalog(pkgs []model.Package, idAliases map[string]string) *pkg.Collection {
	catalog := pkg.NewCollection()
	for _, p := range pkgs {
		catalog.Add(tosanrakshyaPackage(p, idAliases))
	}
	return catalog
}

func tosanrakshyaPackage(p model.Package, idAliases map[string]string) pkg.Package {
	var cpes []cpe.CPE
	...
	out := pkg.Package{
		...
		Licenses:  pkg.NewLicenseSet(tosanrakshyaLicenses(p.Licenses)...),
		Language:  p.Language,
		Type:      p.Type,
		CPEs:      cpes,
		PURL:      p.PURL,
