...
import (
	...
	sanrakshyaJsonModel "github.com/anubhav06/sanrakshya-cli/sanrakshya/format/sanrakshyajson/model"
	"github.com/anubhav06/sanrakshya-cli/sanrakshya/internal/packagemetadata"
)

/*
This method of creating the JSON schema only captures strongly typed fields for the purpose of integrations between sanrakshya
...
func schemaID() jsonschema.ID {
	...
	return jsonschema.ID(fmt.Sprintf("anchore.io/schema/sanrakshya/json/%s", internal.JSONSchemaVersion))
}
...
func assembleTypeContainer(items []any) (any, map[string]string) {
	...
	if len(typesMissingNames) > 0 {
		fmt.Println("the following types are missing JSON names (manually curated in ./sanrakshya/internal/packagemetadata/names.go):")
...
func build() *jsonschema.Schema {
	...
	documentSchema := reflector.ReflectFromType(reflect.TypeOf(&sanrakshyaJsonModel.Document{}))
